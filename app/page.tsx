...
import { Mail, Phone, MapPin, Globe, Twitter, ArrowRight, Network, Search, Crosshair, Radio, ScanLine, ShieldAlert, User, Car, Users, Type, Map, Clock, Moon, Bell, Lock, WifiOff, Database } from 'lucide-react';
...
export default function LandingPage() {
  ...
  const allFeatures = [
    ...
    { icon: Moon, title: 'Night-Time Vision Enhancement', desc: 'Automatically applies CLAHE during night hours for low-light conditions.' },
    ...
    { icon: Lock, title: 'Blockchain Immutable Audit Logs', desc: 'Critical alerts are SHA-256 hashed and chained to previous events, creating tamper-evident records.', isXFactor: true },
    { icon: WifiOff, title: 'Low-Bandwidth Metadata Alerts', desc: 'Detects movement locally and transmits lightweight alert data without sending complete video footage.', isXFactor: true },
    { icon: Database, title: 'Centralized Watchlist Sync', desc: 'Sync wanted-person and stolen-vehicle watchlists across border edge nodes for automatic matching.', isXFactor: true }
  ];
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const scrolledIntoContainer = window.scrollY - (top + window.scrollY);
      const scrollableHeight = height - window.innerHeight;

      if (scrolledIntoContainer < 0) return setActiveIndex(0);
      if (scrolledIntoContainer > scrollableHeight) return setActiveIndex(allFeatures.length - 1);

      const currentIndex = Math.floor((scrolledIntoContainer / scrollableHeight) * allFeatures.length);
  ...
  return (
    <div className="w-full overflow-x-clip bg-brand-bg text-brand-text font-sans"><main className="relative z-10 w-full min-h-screen bg-brand-bg flex flex-col mb-[550px] border-b border-brand-border shadow-[0_30px_60px_rgba(0,0,0,1)] rounded-b-[40px] selection:bg-brand-red selection:text-black">
      ...
      <section id="features" ref={scrollRef} className="relative border-t border-brand-border" style={{ height: '500vh' }}>
        <div className="sticky top-0 flex h-screen flex-col justify-center overflow-hidden">
          <div className="pointer-events-none absolute inset-0 opacity-10 [background-image:linear-gradient(rgba(255,255,255,.1)_1px,transparent_1px),linear-gradient(90deg,rgba(255,255,255,.1)_1px,transparent_1px)] [background-size:54px_54px]" />
          <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_at_top_right,rgba(239,51,72,0.2),transparent_60%)]" />

          <div className="mx-auto grid w-full max-w-7xl grid-cols-1 gap-16 px-6 lg:grid-cols-[400px_1fr] lg:gap-24">
            <div className="h-fit py-10 lg:py-0">
              <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-brand-red/30 bg-brand-red/10 px-3 py-1 text-[10px] font-bold tracking-widest text-brand-red">
                {allFeatures[activeIndex].isXFactor ? 'FIELD RESILIENCE' : 'CORE CAPABILITIES'}
              </div>
              <h2 className="mb-6 text-4xl font-extrabold leading-tight tracking-tight text-brand-text transition-all duration-700 md:text-5xl">
                Unrivaled intelligence at the edge.
              </h2>
              <p className="text-lg font-light text-brand-muted">
              ...
              <div className="mt-12">
                <div className="flex items-center gap-3 text-[10px] font-bold tracking-[0.18em] text-brand-muted">
                  <span className="h-px w-8 bg-brand-red" />
                  <span>SCROLL TO EXPLORE 13 CAPABILITIES</span>
                </div>
                <div className="mt-4 flex items-center gap-4">
                  <span className="font-mono text-sm font-bold tracking-widest text-brand-text">
                    {String(activeIndex + 1).padStart(2, '0')} <span className="text-brand-muted">/ {String(allFeatures.length).padStart(2, '0')}</span>
                  </span>
                  <div className="flex max-w-[200px] flex-wrap gap-2" aria-label={`Capability ${activeIndex + 1} of ${allFeatures.length}`}>
                    {allFeatures.map((_, index) => (
                      <div key={index} className={`h-1 rounded-full transition-all duration-300 ${index === activeIndex ? 'w-6 bg-brand-red' : 'w-2 bg-zinc-800'}`} />
                    ))}
                  </div>
                </div>
              </div>
            ...
            <div className="relative h-[400px] w-full">
              {allFeatures.map((feature, index) => {
                ...
                return (
                  <div key={feature.title} className={`absolute inset-0 w-full transition-all duration-700 ease-in-out ${isActive ? 'pointer-events-auto translate-y-0 opacity-100' : isPast ? 'pointer-events-none -translate-y-12 opacity-0' : 'pointer-events-none translate-y-12 opacity-0'}`}>
                    <div className="flex h-full w-full flex-col justify-center rounded-3xl border border-brand-red/30 bg-brand-card p-10 shadow-[0_0_50px_rgba(255,48,72,0.04)] backdrop-blur-xl md:p-12">
                      <div className="mb-8 flex h-14 w-14 items-center justify-center rounded-2xl border border-brand-red/20 bg-brand-red/15 text-brand-red">
                        <Icon className="h-7 w-7" />
                      </div>
                      <h3 className="mb-4 text-3xl font-bold tracking-wide text-brand-text">{feature.title}</h3>
                      <p className="text-lg font-light leading-relaxed text-brand-muted">{feature.desc}</p>
                    </div>
                  </div>
                );
              })}
