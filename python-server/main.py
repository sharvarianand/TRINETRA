        ...
        try:
            ...
            if task["type"] == "plate" and ocr_reader:
                ...
                try:
                    ...
                    if os.path.exists("watchlist_faces") and len(os.listdir("watchlist_faces")) > 0:
                        df = DeepFace.find(img_path=temp_path, db_path="watchlist_faces", enforce_detection=False, silent=True)
                        if len(df) > 0 and not df[0].empty:
                            matched_identity = df[0].iloc[0]['identity']
                            print(f"MATCHED SUSPECT: {matched_identity}")
